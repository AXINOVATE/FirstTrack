...
var xu_validation = function () {
	...
	var form_validation = function(form_id, success_function){
		...
		if(error == 0){
			...
			$('.disable_div').remove();
			var msg='<div class="alert alert-danger fade in va_form_error"><a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">X</a>Please correct the following errors</div>';
			$(form_id).prepend(msg);
	...
	return {
		...
		fileupload: function(prefix, source, type, ajax_url, accept_files){
			$(source).fileupload({
				...
				done: function (e, data) {var path = data['result']['path'], save_path =$(e.target || e.srcElement).attr('save_path'), call_back =$(e.target || e.srcElement).attr('call_back'), img_path =$(e.target || e.srcElement).attr('message'); $('#'+save_path).val(path);
				if(type=="image"){$('#'+img_path).html('<img src="'+prefix+path+'" style="max-width:150px; border-radius:3px;">');}
				else{$('#'+img_path).html(path.split("/").pop());}
					if(call_back!='' && call_back!='undefined' && call_back){
						//$('#'+save_path).attr('onkeyup',call_back+'("'+path+'","'+ img_path+'","'+ save_path+'");');
						window[call_back](path, img_path, save_path);
						$('#'+save_path).trigger('keyup');
						$('#'+save_path).attr('onKeyup','');
					}
				},
				progressall: function (e, data) {$('#'+$(e.target || e.srcElement).attr('pro_path')).css('width',parseInt(data.loaded / data.total * 100, 10) + '%');}
